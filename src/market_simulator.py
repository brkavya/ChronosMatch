...
class AsyncMarketSimulator:
    ...
    async def generate_order(self):
        """Generates a realistic mock limit order."""
    ...
    async def run(self, max_orders: int = 50):
        ...
        while self.running and count < max_orders:
            ...
            success = self.buffer.write_order(order_id, price, qty, side)

            if success:
                count += 1
                if count % 10 == 0 or count == max_orders:
                    print(f"[Simulator] Injected order #{order_id}: {side} {qty} @ {price}")
            else:
                await asyncio.sleep(0.001)

            await asyncio.sleep(self.interval)

        print(f"[Simulator] Stream complete. Total injected: {count} orders.")
...
if __name__ == "__main__":
    ...
    ring = MmapRingBuffer(test_path, capacity=100)
    sim = AsyncMarketSimulator(ring, rate_per_second=500)

    # Run the event loop
    asyncio.run(sim.run(max_orders=30))
    ...
    while True:
        ...
        if consumed <= 5 or consumed >= 28:
            print(f"  Verified order #{order[0]}: {order[3]} {order[2]} @ {order[1]}")

    print(f"Total verified in ring buffer: {consumed}")
    ring.close()
